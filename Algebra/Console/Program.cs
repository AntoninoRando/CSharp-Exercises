...
Console.WriteLine("Write the congruence(s) or --help for documentation:");
string input = string.Empty;
string line = string.Empty;

// Accepting Input on multiple lines
ConsoleKeyInfo keyInfo;
do
{
    keyInfo = Console.ReadKey();

    if (keyInfo.Key == ConsoleKey.Enter && (Control.ModifierKeys & Keys.Shift) == Keys.Shift)
    {
...
} while (keyInfo.Modifiers != ConsoleModifiers.Shift && keyInfo.Key != ConsoleKey.Enter);
...
void print_help()
{
    ...
    Console.WriteLine("-\tYou can use SHIFT+ENTER to start a new line and better organize your work.");
}